...
class MarketScannerCrew:
    ...
    def __init__(self, target_market: Optional[str] = None, skip_init: bool = False):
        ...
        if target_market is None:
            ...
            active_markets = market_calendar.get_active_markets(datetime.now(pytz.utc), ["US_EQUITY", "CRYPTO"])
        ...
        model_name, api_key = enhanced_gemini_manager.get_llm_for_crewai(
            estimated_requests=8, auto_rotate=True  # Enable automatic key rotation for intensive operations
        )
...
class _MarketScannerCrewProxy:
    ...
    def __getattr__(self, name):
        raise AttributeError(
            f"'_MarketScannerCrewProxy' only supports the 'run' method. " f"Attribute '{name}' is not available."
        )
