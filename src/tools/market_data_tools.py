...
class MarketDataTools:
    ...
    @staticmethod
    def fetch_ohlcv_data(symbol: str, timeframe: str = "1Min", limit: int = 100) -> Dict:
        """
        ...
        """
        try:
            df = alpaca_manager.fetch_historical_bars(symbol=symbol, timeframe=timeframe, limit=limit)

            # Validate data
            validation = MarketDataTools.validate_data_completeness(df)

            if not validation["is_valid"]:
                logger.warning(f"Data validation warning: {validation['issues']}")
    ...
    def validate_data_completeness(df: pd.DataFrame) -> Dict:
        ...
        if df is None or len(df) == 0:
            return {"is_valid": False, "issues": ["DataFrame is empty"]}
        ...
        if len(df) > 1:
            ...
            if expected_diff is not None and not expected_diff.empty:
                ...
                if len(large_gaps) > 0:
                    issues.append(f"Time series gaps detected: {len(large_gaps)} gaps")
    ...
    def get_latest_price(symbol: str) -> Dict:
        ...
        try:
            # Fetch just 1 bar
            df = alpaca_manager.fetch_historical_bars(symbol=symbol, timeframe="1Min", limit=1)
