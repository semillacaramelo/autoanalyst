...
class BacktesterV2:
    ...
    def run(self, symbol: str, strategy_name: str, timeframe: str = '1Day', slippage_percent: float = 0.001, commission_per_trade: float = 1.0, data_feed: Optional[str] = None) -> Dict:
        ...
        for i in range(1, len(data)):
            ...
            if validated_signal['signal'] == 'BUY' and position == 0:
                ...
                trades.append({'date': data.index[i], 'type': 'BUY', 'price': price, 'commission': commission_per_trade})
                position = 1
            elif validated_signal['signal'] == 'SELL' and position == 1:
                price = data['close'].iloc[i] * (1 - slippage_percent)
                trades.append({'date': data.index[i], 'type': 'SELL', 'price': price, 'commission': commission_per_trade})
                position = 0
    ...
    def calculate_performance(self, trades: List[Dict], timeframe: str) -> Dict:
        ...
        for i in range(0, len(trades), 2):
            ...
            trade_pnl = (sell_trade['price'] - buy_trade['price']) - (buy_trade['commission'] + sell_trade['commission'])
        ...
        # Sharpe Ratio
        if np.std(returns) > 0:
            sharpe_ratio = (np.mean(returns) * annualization_factor - self.risk_free_rate) / (np.std(returns) * np.sqrt(annualization_factor))
        else:
            sharpe_ratio = 0
        ...
        if len(downside_returns) > 1 and np.std(downside_returns) > 0:
            sortino_ratio = (np.mean(returns) * annualization_factor - self.risk_free_rate) / (np.std(downside_returns) * np.sqrt(annualization_factor))
        else:
            sortino_ratio = 0
