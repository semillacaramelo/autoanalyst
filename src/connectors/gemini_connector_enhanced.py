...
class ModelQuotaTracker:
    ...
    def __init__(self):
        ...
        self.minute_windows: Dict[str, Dict[ModelTier, deque]] = defaultdict(lambda: {
            ModelTier.FLASH: deque(),
            ModelTier.PRO: deque()
        })
        self.day_windows: Dict[str, Dict[ModelTier, deque]] = defaultdict(lambda: {
            ModelTier.FLASH: deque(),
            ModelTier.PRO: deque()
        })
...
class DynamicModelManager:
    ...
    def get_preferred_models(self) -> Tuple[List[str], List[str]]:
        ...
        return (flash_models[:3] if flash_models else ["gemini-2.5-flash"], 
        ...
        for key_idx, api_key in enumerate(self.api_keys):
            ...
            for model in self.flash_models:
                ...
                if self.quota_tracker.can_use_model(api_key, tier):
                    ...
                    if wait_time == 0:
                        ...
                        logger.info(f"Selected Flash model {model} with key {masked_key}")
            ...
            for model in self.pro_models:
                ...
                if self.quota_tracker.can_use_model(api_key, tier):
                    ...
                    if wait_time == 0:
                        self.quota_tracker.record_request(api_key, tier)
                        logger.info(f"Flash exhausted, using Pro model {model} with key {masked_key}")
                        return (f"gemini/{model}", api_key)
                    elif wait_time and wait_time > 0:
            ...
            logger.warning(f"Both Flash and Pro quotas exhausted on key {masked_key}, trying next key")
