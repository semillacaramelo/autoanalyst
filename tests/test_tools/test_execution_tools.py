...
class TestCalculatePositionSize(unittest.TestCase):
    ...
    def test_normal_position_size(self, mock_settings):
        ...
        result = ExecutionTools.calculate_position_size(
            signal="BUY", current_price=100.0, atr=2.0, account_equity=10000.0
        )
    ...
    def test_zero_atr_fallback(self, mock_settings):
        ...
        result = ExecutionTools.calculate_position_size(
            signal="BUY", current_price=50.0, atr=0.0, account_equity=10000.0  # Zero ATR
        )
    ...
    def test_minimum_one_share(self, mock_settings):
        ...
        result = ExecutionTools.calculate_position_size(
            signal="BUY", current_price=10000.0, atr=100.0, account_equity=1000.0  # Very expensive stock
        )
    ...
    def test_high_volatility_reduces_shares(self, mock_settings):
        ...
        result_low = ExecutionTools.calculate_position_size(
            signal="BUY", current_price=100.0, atr=1.0, account_equity=10000.0  # Low volatility
        )

        # High volatility
        result_high = ExecutionTools.calculate_position_size(
            signal="BUY", current_price=100.0, atr=5.0, account_equity=10000.0  # High volatility
        )
    ...
    def test_error_handling(self, mock_settings):
        ...
        result = ExecutionTools.calculate_position_size(
            signal="BUY", current_price=-100.0, atr=2.0, account_equity=10000.0  # Invalid price
        )
...
class TestCheckPortfolioConstraints(unittest.TestCase):
    ...
    def test_all_constraints_pass(self, mock_alpaca, mock_settings):
        ...
        mock_alpaca.get_positions.return_value = [{"symbol": "SPY", "qty": "10"}, {"symbol": "QQQ", "qty": "5"}]
...
class TestPlaceOrder(unittest.TestCase):
    ...
    def test_successful_buy_order(self, mock_alpaca, mock_constraints):
        ...
        mock_alpaca.place_market_order.assert_called_once_with(symbol="SPY", qty=10, side="BUY")
    ...
    def test_order_rejected_by_constraints(self, mock_constraints):
        ...
        mock_constraints.return_value = {"approved": False, "checks": {"max_positions": {"passed": False}}}
    ...
    def test_order_placement_failure(self, mock_alpaca, mock_constraints):
        ...
        # Mock order failure
        mock_alpaca.place_market_order.return_value = {"status": "rejected", "reason": "Insufficient buying power"}
