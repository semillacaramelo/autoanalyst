...
def validate_risk_management():
    ...
    table.add_row("Max Risk per Trade", f"{settings.max_risk_per_trade * 100}%", "✓")
...
def main():
    ...
    console.print(Panel.fit("[bold yellow]Trading Crew Configuration Validator[/bold yellow]", border_style="yellow"))
    ...
    if passed == total:
        console.print(Panel.fit(f"[bold green]All {total} checks passed! ✓[/bold green]", border_style="green"))
        sys.exit(0)
    else:
        console.print(
            Panel.fit(
                f"[bold red]{passed}/{total} checks passed. Fix errors before running.[/bold red]", border_style="red"
            )
        )
        sys.exit(1)
