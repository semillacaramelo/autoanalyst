            ...
            with ThreadPoolExecutor(max_workers=3) as executor:
                ...
                for symbol in symbol_list:
                    for strategy in strategy_list:
                        ...
                        futures.append(
                            executor.submit(
                                trading_crew.run, symbol=symbol, strategy=strategy, timeframe=timeframe, limit=limit
                            )
                        )
                ...
                for future in futures:
                    ...
                    if result["success"]:
                        ...
                        console.print(
                            f"[red]  ✗ FAILED: {result['symbol']} ({result['strategy']}) - {result['error']}[/red]"
                        )
...
def run_single_crew(symbol, strategy, timeframe, limit):
    ...
    try:
        ...
        if result["success"]:
            console.print(
                Panel.fit("[bold green]✓ Crew execution completed successfully![/bold green]", border_style="green")
            )
            console.print(f"\n[dim]Result: {result['result']}[/dim]")
        else:
            console.print(
                Panel.fit(
                    f"[bold red]✗ Crew execution failed[/bold red]\n[dim]{result.get('error', 'Unknown error')}[/dim]",
                    border_style="red",
                )
            )
...
def scan():
    ...
    """
    console.print(Panel.fit("[bold cyan]Market Scanner Crew[/bold cyan]", border_style="cyan"))
    ...
    try:
        ...
        console.print(
            Panel.fit(f"[bold red]✗ Unexpected error during scan[/bold red]\n[dim]{str(e)}[/dim]", border_style="red")
        )
...
def status(detailed, recommendations):
    ...
    if recommendations:
        ...
        try:
            ...
            status_summary = (
                f"Alpaca Account: {account}, Gemini Key Health: {gemini_manager.key_health_tracker.key_health}"
            )

            recommender = Agent(
                role="AI System Health Analyst",
                goal="Analyze system status and provide recommendations.",
                backstory="An expert AI.",
                llm=llm,
                verbose=False,
            )
            rec_task = Task(
                description=f"Analyze: {status_summary}", expected_output="Recommendations.", agent=recommender
            )
...
def generate_dashboard() -> Layout:
    ...
    layout["footer"].update(
        Panel("[dim]Logs will be displayed here in a future enhancement...[/dim]", title="Logs", border_style="blue")
    )
    return layout
...
def interactive():
    ...
    layout["header"].update(
        Panel(
            f"[bold green]Talos Algo AI - Interactive Dashboard[/bold green]\nLast updated: {datetime.now().ctime()}",
            border_style="cyan",
        )
    )
    layout["body"].update(
        Panel(
            "Welcome! System data will refresh every 5 seconds. Press Ctrl+C to exit.",
            title="Activity",
            border_style="blue",
        )
    )
